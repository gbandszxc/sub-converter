...
拖拽文件到窗口（拖入文件夹会加入其中一级的所有字幕文件，音频、视频等非字幕文件一律不加入）、按 `Ctrl+O`、或点「Add files」按钮添加字幕；
...
但仍拒绝替换源文件。设置项（目标格式、输出位置、指定目录、冲突策略、偏移、BOM、去除媒体后缀、
界面语言、字体）通过 `shared_preferences` 持久化，下次启动恢复。

**退出确认。** 关闭窗口——标题栏 X、Alt+F4 或任务栏关闭——都会先弹出确认对话框，避免进行
中的转换被误操作中止。对话框跟随界面语言；只有确认「退出」才会真正关闭程序。

**去除媒体后缀。** 有些字幕以所属媒体文件命名，例如 `AAAA.wav.vtt`。默认转成 LRC 会得到
`AAAA.wav.lrc`；勾选该选项后，去掉中间的媒体扩展名，输出 `AAAA.lrc`。该选项对任意目标格式
生效，且只去除常见音频/视频扩展名 —— `aac`、`ape`、`flac`、`m4a`、`mp3`、`oga`、`ogg`、
`opus`、`wav`、`wma`、`avi`、`flv`、`m4v`、`mkv`、`mov`、`mp4`、`mpeg`、`mpg`、`ts`、`webm`、
`wmv` —— 因此 `E01.en.srt` 里的语言标签不会被误删。冲突处理（自动改名/覆盖/跳过）作用于去后缀
之后的名字，源文件依然永远不会成为输出。
...
- **路径与文件安全**：冲突策略、媒体后缀剥离与「绝不覆盖源文件」规则；批量转换、进度与单文件失败。
- **UI/控制器**：`AppController` 状态与持久化、主界面 widget 测试、界面语言与字体下拉，以及直接驱动窗口真实 `DropTarget` 回调的拖放测试（多文件拖入、拖入的文件夹展开为其中一级的字幕文件、声明为音频/视频等非字幕类型的文件一律不入列表、空路径忽略、重复项合并）。
...
LICENSE                         Apache License 2.0 全文，本项目的发布许可证。
```
...
本项目以 [Apache License 2.0](LICENSE) 发布。你可以使用、修改并再分发（含商用），只需保留版权
与许可声明、说明你改动了什么；该许可证还包含明确的专利授权。

第三方组件各自遵循其自身许可证，完整文本随每次发布版本一并提供
（`data/flutter_assets/NOTICES.Z`），也可在各项目仓库中查阅。Apache 许可证全文同时展示在安装
程序的许可页，并随应用一起安装到安装目录。
